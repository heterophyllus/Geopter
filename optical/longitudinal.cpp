...
Longitudinal::Longitudinal(OpticalModel* opt_model, Renderer* renderer) :
...
void Longitudinal::plot_lsa(double scale)
{
    ...
    PupilCrd pupil;
    Ray ray;
    Field* fld0 = opt_model_->optical_spec()->field_of_view()->field(0);

    int ref_wvl_idx = opt_model_->optical_spec()->spectral_region()->reference_index();
    int num_wvls = opt_model_->optical_spec()->spectral_region()->wvl_count();
    std::vector<double> l_primes = get_l_primes();
    double img_dst = opt_model_->paraxial_model()->first_order_data().img_dist;

    for(int wi = 0; wi < num_wvls; wi++){
        double wvl = opt_model_->optical_spec()->spectral_region()->wvl(wi)->value();
        Rgb color = opt_model_->optical_spec()->spectral_region()->wvl(wi)->render_color();

        double dfoc = l_primes[wi] - img_dst;
...
{
    double maxfld = opt_model_->optical_spec()->field_of_view()->max_field();
    //renderer_->set_current_cell(0,1);
    renderer_->set_x_axis_range(-scale, scale);
    ...
    for(int wi = 0; wi < num_wvls; wi++){
        ...
        for(int fi = 1; fi < num_data_; fi++) {
            ...
            tmp_fld->set_y(y);
            PupilCrd aim_pt = Trace::aim_chief_ray(*opt_model_, *tmp_fld,wvl);
            tmp_fld->set_aim_pt(aim_pt);
            fy.push_back(y);

            Eigen::Vector2d s_t = Trace::trace_coddington(*opt_model_, *tmp_fld, wvl);

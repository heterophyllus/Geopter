...
std::shared_ptr<Glass> GlassCatalog::glass(std::string glass_name)
{
    if(!glasses_.empty())
    {
        for(auto &g : glasses_)
        {
            if(g->name() == glass_name){
                return g;
            }else if(g->name() == (glass_name + "_" + name_)){
                return g;
            }
        }
    }
...
bool GlassCatalog::load_agf(std::string agf_path)
{
    ...
    {
        if(FileIO::starts_with(line_str, "NM"))
        {
            //NM <glass name> <dispersion formula #> <MIL#> <N(d)> <V(d)> <Exclude Sub> <status> <melt freq>

            std::vector<std::string> line_parts = FileIO::split(line_str,' ');

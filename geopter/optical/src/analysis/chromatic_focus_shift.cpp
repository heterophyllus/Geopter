...
std::shared_ptr<PlotData> ChromaticFocusShift::plot(double lower_wvl, double higher_wvl)
{
    ...
    auto ref_prx_ray = tracer->TraceParaxialRayFromObject(y0, u0, ref_wvl_val_);
    double ref_u_prime = ref_prx_ray->Back().u_prime;
    double ref_y = ref_prx_ray->Back().y;
    double ref_l_prime = -ref_y/ref_u_prime;
    ...
    for(int i = 0; i < num_data; i++) {
        ...
        auto prx_ray = tracer->TraceParaxialRayFromObject(y0, u0, wvl);
        double y = prx_ray->Back().y;
        double u_prime = prx_ray->Back().u_prime;
    ...
    graph->SetName("focus shift");

    auto plot_data = std::make_shared<PlotData>();
    plot_data->SetTitle("Chromatic Focus Shift");
    plot_data->SetXLabel("Wvl(nm)");
    plot_data->SetYLabel("FocusShift");
